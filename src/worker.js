...
async function handleResponse(actionData, response) {
        ...
        if (version === "v1") {
          if (action === "register") {
            ...
            try {
              ...
              const appsScriptId = Array.from(data).filter((e) => e.startsWith("id: "))[0].substring(4);
              if (data.has("Google-Apps-Script") && appsScriptId.length > 0) {
                if (!(await env.GOOGLE_FORMS.get(`registrationKey-${reqBody.key}`))) return new Response("Invalid or expired key.", { status: 404, headers: corsHeaders });
                await env.GOOGLE_FORMS.put(`registrationData-${reqBody.key}`, JSON.stringify({ ...reqBody, url, appsScriptId }), { cacheTtl: 600 }); // 10 min cache
            ...
            try {
              ...
              if (code === data.code) {
                ...
                if (data.url.startsWith(registrationData.fixedUrl) || data.url.startsWith("/" + registrationData.fixedUrl)) {
                  await env.GOOGLE_FORMS.put(`data-${data.url}`, JSON.stringify({ handler: registrationData.handler, handlerData: registrationData.handlerData, appsScriptId: data.appsScriptId, formData: data.formData }));
            ...
            if (actionData) {
              try {
                ...
                if (data.has("Google-Apps-Script") && data.has(`id: ${actionData.appsScriptId}`)) {
                  await handleResponse(actionData, reqBody);
